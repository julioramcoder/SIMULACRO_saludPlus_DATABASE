...
function buildSummary(appointments = []) {
  ...
  for (const a of appointments) {
    ...
    // aqui vamos contando cuantas veces aparece cada especialidad
  ...
  for (const [spec, c] of specialtyCount.entries()) {
    if (c > best) {
      best = c;
      mostFrequentSpecialty = spec;
    }
  }
...
export async function getPatientHistoryByEmail(email) {
  ...
  // aqui usamos la funcion que construimos arriba para calcular el resumen del historial
  ...
  return {
    ...
    summary,
    // devolvemos el resumen calculado
  };
}